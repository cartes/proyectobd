...
<div {{ $attributes->merge(['class' => 'flex gap-3']) }}>
    ...
    {{-- Twitter --}}
    <a href="https://twitter.com/intent/tweet?url={{ urlencode($url) }}&text={{ urlencode($title) }}" target="_blank"
    ...
    <a href="https://wa.me/?text={{ urlencode($title . ' ' . $url) }}" target="_blank" rel="noopener noreferrer"
        class="flex items-center gap-2 px-4 py-2 bg-green-600 text-white rounded-lg hover:bg-green-700 transition-colors md:hidden">
        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
            <path
